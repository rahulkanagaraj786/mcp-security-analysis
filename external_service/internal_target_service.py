"""
Internal Target Service - Contains Secrets

This service simulates an internal-only service that contains secrets.
It should only be accessible from localhost/internal network.
The MCP server can access this, but external attackers cannot.

This is the TARGET of SSRF attacks - the service that contains the secrets
that attackers want to steal.

Port: 8080 (internal only, not publicly accessible)
"""
...
app = FastAPI(title="Internal Target Service", description="Internal service with secrets (SSRF target)")
...
async def log_requests(request: Request, call_next):
    ...
    # Warn if accessed from non-localhost
    if client_ip not in ["127.0.0.1", "localhost", "::1"]:
        print(f"[Internal Service] ⚠️  WARNING: Access from non-localhost: {client_ip}")
    else:
        print(f"[Internal Service] {timestamp} - {request.method} {request.url.path} from {client_ip}")
...
async def root():
    ...
    return {
        "service": "Internal Target Service",
        "description": "This service contains SECRETS and should only be accessible from internal network",
...
async def admin_config():
    ...
    return {
        ...
        "warning": "This data should never be exposed to external users!"
    }
...
async def internal_keys():
    ...
    return {
        ...
        "warning": "These secrets should never be exposed!"
    }
...
async def instance_metadata():
    ...
    return {
        ...
        "warning": "Metadata should only be accessible from within the instance!"
    }
...
async def health():
    ...
    return {
        ...
        "warning": "Health data should not be exposed externally!"
    }
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("  Internal Target Service - Contains Secrets")
    print("="*70)
    print("  Port: 8080")
    print("  Purpose: Simulate internal-only service with secrets for SSRF attacks")
    print("  ⚠️  This service contains SECRETS")
    print("  ⚠️  Should only be accessible from localhost/internal network")
    print("  Endpoints:")
    ...
    # Run on 127.0.0.1 only (not 0.0.0.0) to simulate internal-only access
