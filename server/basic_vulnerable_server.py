"""
Basic Vulnerable MCP Server - Vulnerable to SSRF Redirect Attacks
"""
import asyncio
import logging
import requests
...
class BasicVulnerableMCPServer:
    ...
    async def vulnerable_get_weather(self, arguments: Dict[str, Any]) -> CallToolResult:
        """VULNERABLE: Weather service that can be exploited for SSRF redirect attacks"""
        city = arguments.get("city", "")
        
        logger.warning(f"VULNERABLE: Getting weather for city: {city}")
        
        # VULNERABILITY: Server constructs URL based on user input without validation
        # In a real scenario, this might be a legitimate weather API
        weather_url = f"http://127.0.0.1:8001/weather?city={city}"
        
        try:
            # VULNERABILITY: requests.get automatically follows redirects
            # No validation of redirect destinations
            response = requests.get(weather_url, timeout=10, allow_redirects=True)
    ...
    async def vulnerable_fetch_url(self, arguments: Dict[str, Any]) -> CallToolResult:
        """VULNERABLE: Direct URL fetching with no validation"""
